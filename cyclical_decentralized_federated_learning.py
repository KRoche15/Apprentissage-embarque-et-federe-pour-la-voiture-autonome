...
##hyper-parameters for cyclical decentralized federated learning
num_clients = 5
num_selected = np.array([0,2,3,4,5])
seed = 3
num_rounds = 100
epochs = 5
...
with open(os.path.join(cfg.TRAIN.CHECKPOINT_SAVE_PATH, 'decentralized_results.txt'), 'a+') as result_file:
    result_file.write("DFL : num_selec = 6 / epoch=5 / round=100 \n")
...
for r in range(1,num_rounds+1):
    ...
    for i in num_selected:
        losses=client_update(model,opt,train_loader[i],epoch=epochs)
        loss+=losses
        losses_train.append(losses)
        
    val_loss, val_acc , val_rmse = test(model, val_loader)
    accuracy.append(val_acc)
    mesure_rmse.append(val_rmse)
    
    test_loss, test_acc , test_rmse = test(model, test_loader)
    
    print('%d-th round' %r)
    print('Validation : average train loss : %0.3g | test loss : %0.3g | test acc : %0.1f | rmse : %0.4f' % (loss / len(num_selected), val_loss, val_acc*100, val_rmse))
    print('Test : average train loss : %0.3g | test loss : %0.3g | test acc : %0.1f | rmse : %0.4f' % (loss / len(num_selected), test_loss, test_acc*100, test_rmse))
    torch.save(model.state_dict(), os.path.join(cfg.TRAIN.CHECKPOINT_SAVE_PATH,'ckpt_' + str(checkpoint_count)) + ".ckpt")
    checkpoint_count += 1
    print(" ")
    with open(os.PATH.join(cfg.TRAIN.CHECKPOINT_SAVE_PATH, 'decentralized_results.txt'), 'a+') as result_file:
        result_file.write(f"Val: checkpoint_{checkpoint_count} : train_loss = {loss / len(num_selected)} , val_loss ={val_loss}, val_accuracy = {val_acc}, val_rmse = {val_rmse} \n")
        result_file.write(f"Test: checkpoint_{checkpoint_count}: train_loss = {loss / len(num_selected)} , test_loss ={test_loss}, test_accuracy = {test_acc}, test_rmse = {test_rmse} \n")
        result_file.write(f"\n")
